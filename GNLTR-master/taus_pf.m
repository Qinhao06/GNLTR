...
area_GTVLRR = (-sum((taus(1:end-1)-taus(2:end)).*(PF_gtvlrr(2:end)+PF_gtvlrr(1:end-1))/2))/r_max
NBOXPLOT(:,8)=f_show(:);
...
area_LRASR = (-sum((taus(1:end-1)-taus(2:end)).*(PF40(2:end)+PF40(1:end-1))/2))/r_max
...
area_TPCA = (-sum((taus(1:end-1)-taus(2:end)).*(PF7(2:end)+PF7(1:end-1))/2))/r_max
NBOXPLOT(:,7)=f_show(:);
...
area_RX = (-sum((taus(1:end-1)-taus(2:end)).*(PF3(2:end)+PF3(1:end-1))/2))/r_max
...
area_LSMAD = (-sum((taus(1:end-1)-taus(2:end)).*(PF_41(2:end)+PF_41(1:end-1))/2))/r_max
...
NBOXPLOT(:,3)=f_show(:);
area_RPCA = (-sum((taus(1:end-1)-taus(2:end)).*(PF2(2:end)+PF2(1:end-1))/2))/r_max
% 
...
area_TLRR = (-sum((taus(1:end-1)-taus(2:end)).*(PF10(2:end)+PF10(1:end-1))/2))/r_max
...
% end
% area_LREN = (-sum((taus(1:end-1)-taus(2:end)).*(PF21(2:end)+PF21(1:end-1))/2))/r_max
% f_show=reshape(r_new,[H,W]);
% f_show=(f_show-min(f_show(:)))/(max(f_show(:))-min(f_show(:)));
...
area_dec_taus = (-sum((taus(1:end - 1) - taus(2:end)) .* (PF11(2:end) + PF11(1:end - 1)) / 2))/r_max
f_show=reshape(r_new,[H,W]);
f_show=(f_show-min(f_show(:)))/(max(f_show(:))-min(f_show(:)));
imwrite(f_show,'DECNN.jpg');
...
%     r_max = max(r_new(:));
    ...
    area_GNLTA = (-sum((taus(1:end - 1) - taus(2:end)) .* (PF20(2:end) + PF20(1:end - 1)) / 2))/r_max
f_show=reshape(r_new,[H,W]);
f_show=(f_show-min(f_show(:)))/(max(f_show(:))-min(f_show(:)));
...
figure,
plot(taus, PF3, 'c-', 'LineWidth', 2);hold on;
plot(taus, PF2, 'r-', 'LineWidth', 2);
plot(taus, PF_41, 'y-', 'LineWidth', 2);
plot(taus, PF40, 'm-', 'LineWidth', 2);
% plot(PF4, PF4, 'c-', 'LineWidth', 2);
plot(taus, PF7, 'Color',[0,0.75,0.25], 'LineWidth', 2);
plot(taus, PF_gtvlrr, 'Color',[0.5,0.5,1], 'LineWidth', 2);
% plot(PF0, PF0, 'g-', 'LineWidth', 2);  
plot(taus, PF10, 'g-', 'LineWidth', 2);
plot(taus, PF21, 'Color',[0,0.75,1], 'LineWidth', 2);
plot(taus, PF11, 'k-', 'LineWidth', 2);
plot(taus, PF20, 'b-', 'LineWidth', 4);
hold off;
xlabel('False alarm rate'); ylabel('Probability of detection');
...
figure, 
semilogx(taus, PF3, 'c-', 'LineWidth', 3); hold on;
semilogx(taus, PF2, 'r-', 'LineWidth', 2);  
semilogx(taus, PF_41, 'y-', 'LineWidth', 2);
semilogx(taus, PF40, 'm-', 'LineWidth', 2);
% semilogx(PF4, PF4, 'c-', 'LineWidth', 2);
semilogx(taus, PF7,  'Color',[0,0.75,0.25], 'LineWidth', 2);
semilogx(taus, PF_gtvlrr, 'Color',[0.5,0.5,1], 'LineWidth', 2);
% semilogx(PF0, PF0, 'g-', 'LineWidth', 2);
semilogx(taus, PF10, 'g-', 'LineWidth', 2);
semilogx(taus, PF21, 'Color',[0,0.75,1], 'LineWidth', 2);
semilogx(taus, PF11, 'k-', 'LineWidth', 2);
semilogx(taus, PF20, 'b-', 'LineWidth', 4);
hold off;
xlabel('False alarm rate'); ylabel('Probability of detection');
% legend('GRXD','RPCA-RX','LSMAD','LRASR','TDAD','TPCA','GTVLRR','PTA');
% legend('GRXD','RPCA-RX','LSMAD','LRASR','TPCA','GTVLRR','PTA', 'PCA-TLRSR', 'GNLTA');
