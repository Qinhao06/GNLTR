function [L,S,trank,obj,err,iter] = trpca_tnn_w(X,lambda,opts, fun, gamma)
...
for iter = 1 : max_iter
    ...
    Sk = S;
    % update L
    [L,tnnL,trank] = prox_wtnn(-S+X-Y/mu, hfun_sg, gamma, lambda2, 1/mu1);
    % update S
%     S = prox_l1(-L+X-Y/mu,lambda/mu);
 S = solve_l1l1l2(-L+X-Y/mu,lambda/mu);
