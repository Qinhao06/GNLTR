...
def main():
    Folder_name = 'detection_results-' + 'beach1'
    # file_name_list = ['abu-urban-1', 'abu-urban-2', \
    #                   'abu-urban-4', 'abu-urban-5', 'abu-beach-1', 'abu-beach-2', 'abu-beach-3', 'abu-beach-4',
    #                   'HYDICE_urban']
    file_name_list = [ 'abu-beach-1']
    for i in range(0, len(file_name_list)):
        ...
        # load_fn = './datasets/avirs/Sandiego_new.mat'
        # load_data = sio.loadmat(load_fn)
        # load_matrix = load_data['hsi']
        ...
        # load_fn = './datasets/avirs/Sandiego_gt.mat'
        # load_data = sio.loadmat(load_fn)
        # anomal_target_map = load_data['hsi_gt']
        ...
        model_lren = LREN([400, hidden_nodes], tf.nn.tanh, est_hiddens=[60, clusters_num], est_activation=tf.nn.tanh,
                          est_dropout_ratio=0.5, epoch_size=1000, minibatch_size=int(4096)
                          )
        ...
        auc_score = roc_auc_score(
            anomal_target_map.reshape([anomal_target_map.shape[0] * anomal_target_map.shape[1], 1]), energy)
        _, fpr_auc = false_alarm_rate(
            anomal_target_map.reshape([anomal_target_map.shape[0] * anomal_target_map.shape[1], 1]), energy)
        ...
        plt.title(
            'cluster centers={} hidden nodes={}\nAUC={}, FPR{},lam={}'.format(clusters_num, hidden_nodes, auc_score,
                                                                              fpr_auc, Lam))
        plt.savefig(
            './' + Folder_name + '//' + file_name + '/' + file_name + 'Energy-map-cluster centers={}hidden nodes={},lam={}.png'.format(
                clusters_num, hidden_nodes, Lam))
        # save_fn = './' + Folder_name + '//' + file_name + '/' + file_name + 'Energy-map-cluster centers={}hidden nodes={},lam={}.mat'.format(
        #     clusters_num, hidden_nodes, Lam)
        save_fn = './' + Folder_name + '//' + file_name + '/' + file_name + 'Energy-map-cluster.mat'.format(
            clusters_num, hidden_nodes, Lam)
        save_array = energy.reshape(anomal_target_map.shape)
        sio.savemat(save_fn, {'array': save_array})

        save_fn2 = './' + Folder_name + '//' + file_name + '/' + file_name + 'Energy.mat'
        # save_array2 = energy = ((energy - energy.min()) /
        #                        (energy.max() - energy.min()))
        sio.savemat(save_fn2, {'r_new': energy})
        ...
        plt.close('all')
        time_end = time.time()
        txtfile.write("time {} s".format(time_end - time_start))
        print(file_name)
