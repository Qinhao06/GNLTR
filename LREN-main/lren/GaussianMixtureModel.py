...
class GaussianMixtureModel:
    ...
    def Gaussian_Mixture_Model_Parameter_Estimation(self, z, gamma):
        with tf.compat.v1.variable_scope("Gaussian_Mixture_Model"):
            ...
            min_vals = tf.compat.v1.diag(tf.ones(n_features, dtype=tf.float32)) * 1e-6
            self.L_Cholesky = tf.compat.v1.cholesky(Sigma + min_vals[None,:,:])
    ...
    def Calculate_Energy(self, z):
        ...
        with tf.compat.v1.variable_scope("Gaussian_Mixture_Model_Energy"):
            ...
            v = tf.compat.v1.matrix_triangular_solve(self.L_Cholesky, tf.transpose(z_centered, [1, 2, 0]))  # kli
            log_det_Sigma = 2.0 * tf.reduce_sum(tf.compat.v1.log(tf.compat.v1.matrix_diag_part(self.L_Cholesky)), axis=1)
            d = z.get_shape().as_list()[1]
            logits = tf.compat.v1.log(self.Phi[:,None]) - 0.5 * (tf.reduce_sum(tf.square(v), axis=1)
                + d * tf.compat.v1.log(2.0 * np.pi) + log_det_Sigma[:,None])
    ...
    def Cov_Diag_Loss(self):
        with tf.compat.v1.variable_scope("Gaussian_Mixture_Model_Diag_Loss"):
            diag_loss = tf.reduce_sum(tf.divide(1, tf.compat.v1.matrix_diag_part(self.Sigma)))
