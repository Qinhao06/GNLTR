...
class SpectralMappingNet:
    ...
    def __init__(self, hidden_layer_sizes, activation=tf.compat.v1.nn.tanh):
    ...
    def OPD(self, ri, rj):
        ...
        with tf.compat.v1.variable_scope("OPD"):
            ...
            diag1 = tf.compat.v1.diag_part(tf.matmul(tf.transpose(ri), ri))
            diag2 = tf.compat.v1.diag_part(tf.matmul(tf.transpose(rj), rj))
